...
// CONFIGURAÇÃO DA API
// ============================================================
const URL_API_GS = "https://script.google.com/macros/s/AKfycbwEKcJhBT30ipZmcS810Oa9IV6S2EP_C_GRvgC2ZzLEhPwhbLk7wyuPj4kI6s9UBlhr5w/exec";
...
async function renderizarCartoes() {
    ...
    state.dadosCartoes.forEach(item => {
        ...
        totais[item.responsavel] += Number(item.qtd) || 0;
    });
...
async function excluirMesCartao(data) {
    ...
    if (!confirm(`Excluir TODOS os cartões do mês ${mes}/${ano}?`)) return;
...
async function carregarListaResponsaveisNoModal() {
    ...
    state.responsaveis.forEach(nome => {
        ...
        texto.textContent = nomeLimpo;
        const botao = document.createElement('button');
        botao.textContent = '×';
        botao.onclick = () => deletarResponsavel(nomeLimpo);
        botao.style.cssText = 'border:none; background:transparent; color:#ff4757; font-weight:bold; cursor:pointer;';
...
function removerEntrega(botao) {
    ...
    itens.forEach((item, idx) => {
        ...
        const numero = item.querySelector('div:first-child div:first-child');
...
function atualizarContador() {
    const contador = document.getElementById('mult-contador');
    if (!contador) return;
    contador.textContent = `${contadorEntregas} ${contadorEntregas === 1 ? 'entrega' : 'entregas'}`;
}
...
function limparCamposNomeEndereco() {
    const nomes = document.querySelectorAll('#mult-lista-entregas .nome-input');
    const enderecos = document.querySelectorAll('#mult-lista-entregas .endereco-input');
    nomes.forEach(nome => { nome.value = ''; nome.style.borderColor = '#ddd'; });
    enderecos.forEach(endereco => { endereco.value = ''; endereco.style.borderColor = '#ddd'; });
}
...
function normalizeString(s) {
    if (!s && s !== 0) return "";
    return s.toString().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/\s+/g, ' ').toLowerCase().trim();
}

function headerToId(lbl) {
    if (!lbl && lbl !== 0) lbl = "";
    return lbl.toString().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-zA-Z0-9]+/g, '_').replace(/^_+|_+$/g, '').replace(/_+/g, '_').toUpperCase();
}
...
async function abrirModalCesta() {
    ...
    if (scannerInput) setTimeout(() => { scannerInput.focus(); }, 300);
}
...
async function carregarTiposCesta() {
    ...
    const input = document.getElementById('cesta-tipos-input');
    if (input) input.value = cestaState.types.join(', ');
}
...
async function salvarTiposCesta() {
    const input = document.getElementById('cesta-tipos-input');
    const tiposStr = input.value.trim();
    const tipos = tiposStr.split(',').map(t => t.trim().toUpperCase()).filter(t => t);
    await postParaGoogleSheets('salvarTiposCesta', { tipos });
...
async function renderizarPendentesCestaHome() {
    try {
        ...
        console.error(e);
        const container = document.getElementById('cesta-pendentes-home');
        if (container) container.innerHTML = '<span style="color:#888;">Erro ao carregar pendentes.</span>';
    }
}

async function abrirModalCestaComNome(nome) {
    abrirModal('modal-cesta');
    setTimeout(() => { buscarEPreencherCesta(nome, false); }, 300);
}
...
async function deletarPendente(linha, nome) {
    ...
    await postParaGoogleSheets('deletarMoradorCesta', { linha });
...
document.addEventListener('click', e => {
    const suggestions = document.getElementById('cesta-suggestions');
    if (!suggestions) return;
    if (!suggestions.contains(e.target) && e.target !== inputCesta) suggCesta.style.display = 'none';
});

const cestaBtnSearch = document.getElementById('cesta-btnSearch');
if (cestaBtnSearch) {
...
async function buscarEPreencherCesta(nome, isQRCode = false) {
    try {
        const resp = await fetchFromGS('buscarMoradorCesta', { nome });
        if (!resp || !resp.dados) {
            alert(isQRCode ? "❌ Morador não encontrado na planilha." : "❌ Morador não encontrado.");
            return;
        }
        ...
        console.error(e);
        alert(isQRCode ? "Erro ao buscar os dados via QR Code." : "Erro ao buscar os dados.");
    }
}
...
function renderFormCesta(dadosArray) {
    ...
    const fields = document.getElementById('cesta-fields');
    const monthsContainer = document.getElementById('cesta-monthsContainer');
    fields.innerHTML = '';
    ...
    dadosArray.forEach(item => {
        ...
        if (isMonth) {
            ...
            inputMonth.addEventListener('click', function () {
                ...
                if (confirm(`Marcar mês ${label} como entregue hoje (${diaStr}/${mesStr})?`)) {
            ...
            if (label.trim().toUpperCase() === 'TIPO') {
                ...
                wrapper.innerHTML = `<label>${label}</label><input class="field" id="${id}">`;
...
function atualizarMesesUICesta() {
    ...
    if (statusInput) {
        ...
        statusInput.value = isPago ? 'ENTREGUE' : 'PENDENTE';
        statusInput.style.backgroundColor = isPago ? '#16a34a' : '#dc2626';
...
async function salvarCestaAutomatico() {
    ...
    await postParaGoogleSheets('salvarMoradorCesta', { linha: cestaState.currentLine, payload });
    renderizarPendentesCestaHome();
}
...
async function abrirCameraCesta() {
    ...
    if (container.style.display === 'block') {
        if (cameraHtml5QrCesta) {
            try { await cameraHtml5QrCesta.stop(); await cameraHtml5QrCesta.clear(); } catch (e) {}
    ...
    try {
        await cameraHtml5QrCesta.start({ facingMode: 'environment' }, { fps: 10, qrbox: { width: 200, height: 200 } }, onScanSuccessCesta);
    } catch (err) {
...
function onScanSuccessCesta(decodedText) {
    ...
    if (cameraHtml5QrCesta) {
        try { cameraHtml5QrCesta.stop().catch(() => {}); cameraHtml5QrCesta.clear().catch(() => {}); } catch (e) {}
        cameraHtml5QrCesta = null;
    }
    if (container) { container.style.display = 'none'; container.innerHTML = ''; }
    if (btn) btn.innerText = '📷 Escanear Carteirinha';
    const nome = decodedText.trim();
    if (!nome) return;
    const input = document.getElementById('cesta-search');
...
async function gerarCarteirinha() {
    ...
    try {
        cestaState.qrCodeInstance = new QRCode(qrContainer, { text: nome, width: 75, height: 75, colorDark: "#4a7c2e", colorLight: "#ffffff", correctLevel: QRCode.CorrectLevel.H });
    } catch (e) {
...
function formatarRGPrint(i) {
    i.value = i.value.replace(/\D/g, '').replace(/(\d{1,2})(\d{3})(\d{3})(\d{1})$/, '$1.$2.$3-$4');
}
...
function buscarSugestoesEndereco() {
    ...
    debounceTimerEndereco = setTimeout(async () => {
        try {
            ...
            const resultados = await fetchFromGS('buscarEnderecos', { q: removerAcentos(queryOriginal) }, searchController.signal);
            const querySemAcento = removerAcentos(queryOriginal);
            const resultadosFiltrados = (resultados || []).filter(item => {
                const enderecoSemAcento = removerAcentos(String(item.endereco || '').toUpperCase());
...
function exibirSugestoes(container, resultados) {
    ...
    resultados.forEach(item => {
        ...
        small.textContent = `${item.bairro || ''} - ${item.uf || ''} (CEP: ${item.cep || 'N/I'})`;
...
document.addEventListener('click', function (e) {
    const container = document.getElementById('address-suggestions');
    const input = document.getElementById('print-endereco');
    if (container && input && !container.contains(e.target) && e.target !== input) container.style.display = 'none';
});
...
function extrairNomeRua(endereco) {
    ...
    rua = rua.replace(/^(RUA|R\.|R|TRAVESSA|TRAV|TRV|AVENIDA|AV|ALAMEDA|PRACA|LARGO|RODOVIA|VILA|BECO)\s+/i, '');
    return rua.trim();
}
